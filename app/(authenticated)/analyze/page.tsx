...
export default function AnalyzePage() {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    setError(null)
    try {
      // 1. Appel API pour l'analyse
      const result = await apiClient.analyzeAudio(file, user?.id)

      // 2. Création d'une URL locale pour l'audio (pour lecture immédiate)
      const localAudioUrl = URL.createObjectURL(file)

      // 3. On injecte cette URL dans le rapport avant de l'afficher
      setReport({
        ...result,
  ...
  const handleRecordingComplete = async (audioBlob: Blob) => {
    ...
    setError(null)
    try {
      // 1. Appel API pour l'analyse de l'enregistrement
      const result = await apiClient.analyzeRecording(audioBlob, user?.id)

      // 2. Création d'une URL locale pour l'enregistrement
      const localAudioUrl = URL.createObjectURL(audioBlob)

      // 3. Injection dans le rapport
      setReport({
        ...result,
        audioUrl: localAudioUrl
      })
  ...
  const handleReset = () => {
    // Nettoyage de l'URL objet pour libérer la mémoire si elle existe
    if (report?.audioUrl && report.audioUrl.startsWith('blob:')) {
        URL.revokeObjectURL(report.audioUrl)
    }
    setReport(null)
    setError(null)
  }
